...
export default function init() {
  ...
  $.fn.perpetualX = function (options) {
    ...
    TweenMax.fromTo(
      ...
      speed,
      { x: 0 },
      { x: parentWidth, ease: Linear.easeNone, repeat: -1 }
    ).progress(0)
    TweenMax.fromTo(
      $object2,
      speed,
  ...
  if ($('#intro').length) {
    ...
    TweenMax.to('#intro .wave-1 .dot.one', 70, {
      bezier: { values: line1, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    }).progress(0)
    TweenMax.to('#intro .wave-1 .dot.two', 70, {
      bezier: { values: line1, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    }).progress(0.25)
    TweenMax.to('#intro .wave-1 .dot.three', 70, {
      bezier: { values: line1, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    }).progress(0.5)
    TweenMax.to('#intro .wave-1 .dot.four', 70, {
      bezier: { values: line1, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    }).progress(0.75)
    ...
    TweenMax.to('#intro .wave-2 .dot.one', 45, {
      bezier: { values: line2, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    }).progress(0)
    TweenMax.to('#intro .wave-2 .dot.two', 45, {
      bezier: { values: line2, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    }).progress(0.25)
    TweenMax.to('#intro .wave-2 .dot.three', 45, {
      bezier: { values: line2, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    }).progress(0.5)
    TweenMax.to('#intro .wave-2 .dot.four', 45, {
      bezier: { values: line2, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    }).progress(0.75)
    ...
    $(window)
      .on('resize', function () {
        TweenMax.fromTo(
          ...
          50,
          { width: 0 },
          { width: viewportWidth, ease: Linear.easeNone, repeat: -1 }
        )
      })
      .trigger('resize')
  }
  ...
  $('*[data-modal]').on('click', function () {
    ...
    if (modalID == 'person-modal') {
      if ($(this).hasClass('has-bio')) {
        ...
        $('#person-modal')
          .find('.person-link')
          .attr('href', $(this).find('.person-link').attr('href'))
        $('#person-modal')
          .find('.photo')
          .attr('src', $(this).find('.photo').attr('src'))
  ...
  function coinMove(element, duration, wait) {
    ...
    coinAnimation
      ...
      .set(element, { className: '+=active' }, duration * 0.35)
  }
  ...
  function documentRight(element, wait) {
    ...
    documentAnimation
      .fromTo(
        ...
        4,
        { x: 528, y: 34 },
        { x: 270, y: 34, ease: Power1.easeInOut }
      )
      .to(element, 1.5, {
        y: 210,
        ease: Power1.easeIn,
  ...
  function documentLeft(element, wait) {
    ...
    documentAnimation
      .fromTo(
        ...
        4,
        { x: -62, y: 34 },
        { x: 210, y: 34, ease: Power1.easeInOut }
      )
      .to(element, 1.5, {
        y: 210,
        ease: Power1.easeIn,
  ...
  if ($('#token-loud-animation-2').length) {
    ...
    var coinPath = MorphSVGPlugin.pathDataToBezier(
      '#token-loud-animation-2 .coin-path.one path'
    )
    TweenMax.to('#token-loud-animation-2 .coin.one', 2, {
      bezier: { values: coinPath, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    })

    var coinPath2 = MorphSVGPlugin.pathDataToBezier(
      '#token-loud-animation-2 .coin-path.two path'
    )
    TweenMax.to('#token-loud-animation-2 .coin.two', 2.2, {
      bezier: { values: coinPath2, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    })

    var coinPath3 = MorphSVGPlugin.pathDataToBezier(
      '#token-loud-animation-2 .coin-path.three path'
    )
    TweenMax.to('#token-loud-animation-2 .coin.three', 1.8, {
      bezier: { values: coinPath3, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    })

    var coinPath4 = MorphSVGPlugin.pathDataToBezier(
      '#token-loud-animation-2 .coin-path.four path'
    )
    TweenMax.to('#token-loud-animation-2 .coin.four', 2.2, {
      bezier: { values: coinPath4, type: 'cubic' },
      ease: Linear.easeNone,
      repeat: -1
    })
  }
  ...
  function onSuccess() {
    ...
    notificationContent.innerHTML =
      'Email Sent! We’ll get back with you shortly.'
  ...
  function onFailure() {
    ...
    notificationContent.className = 'content'
    notificationContent.innerHTML =
      'Sorry, we were unable to send your email. Please try again.'
    notificationContainer.appendChild(notificationContent)
      ...
      success: function (data) {
        if (data.result != 'success') {
          ...
          if (message === 'Thank you for subscribing!') {
            message =
              "Thanks - You're on the waitlist! We'll let you know when we publicly launch."
          }
